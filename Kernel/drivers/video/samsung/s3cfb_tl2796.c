...
static ssize_t gamma_table_show(struct device *dev, struct device_attribute *attr,
				char *buf) {
	int point;
	int color;
...
static ssize_t gamma_table_store(struct device *dev, struct device_attribute *attr,
	...
	int i;
	for ( i = 0; ! sscanf(buf, "%u,%u,%u%n", &values[0], &values[1], &values[2],
			&bytes_read) && i <= 1024; i++)
		buf += 1;

	// read the standard gamma table format
	while (sscanf(buf, "%u,%u,%u%n", &values[0], &values[1], &values[2],
			&bytes_read) && point < lcd_->data->gamma_table_size) {
		buf += bytes_read;
		color = 0;
...
static ssize_t apply_custom_brightness_gammas_store(struct device *dev, struct device_attribute *attr,
				const char *buf, size_t size) {
	int bytes_read = 0;
	int user_values[3];
	u16 gamma_regs[27];
	...
	while (sscanf(buf, "%d,%d,%d%n", &user_values[0], &user_values[1], &user_values[2],
			&bytes_read) && i <= 5) {
...
static ssize_t red_multiplier_show(struct device *dev, struct device_attribute *attr,
				char *buf) {
	return sprintf(buf, "%u\n", lcd_->color_mult[0]);
}

static ssize_t red_multiplier_original_show(struct device *dev, struct device_attribute *attr,
				char *buf) {
	return sprintf(buf, "%u\n", original_color_adj_mults[0]);
}

static ssize_t red_multiplier_store(struct device *dev, struct device_attribute *attr,
				const char *buf, size_t size) {
	u32 value;
...
static ssize_t green_multiplier_show(struct device *dev, struct device_attribute *attr,
				char *buf) {
	return sprintf(buf, "%u\n", lcd_->color_mult[1]);
}

static ssize_t green_multiplier_original_show(struct device *dev, struct device_attribute *attr,
				char *buf) {
	return sprintf(buf, "%u\n", original_color_adj_mults[1]);
}

static ssize_t green_multiplier_store(struct device *dev, struct device_attribute *attr,
				const char *buf, size_t size) {
	u32 value;
	if (sscanf(buf, "%u", &value) == 1) {
...
static ssize_t blue_multiplier_show(struct device *dev, struct device_attribute *attr,
				char *buf) {
	return sprintf(buf, "%u\n", lcd_->color_mult[2]);
}

static ssize_t blue_multiplier_original_show(struct device *dev, struct device_attribute *attr,
				char *buf) {
	return sprintf(buf, "%u\n", original_color_adj_mults[2]);
}

static ssize_t blue_multiplier_store(struct device *dev, struct device_attribute *attr,
				const char *buf, size_t size) {
u32 value;
if (sscanf(buf, "%u", &value) == 1) {
...
static ssize_t red_v1_offset_show(struct device *dev, struct device_attribute *attr,
				char *buf) {
	return sprintf(buf, "%d\n", hacky_v1_offset[0]);
}

static ssize_t red_v1_offset_store(struct device *dev, struct device_attribute *attr,
				const char *buf, size_t size) {
	int value;
...
static ssize_t green_v1_offset_show(struct device *dev, struct device_attribute *attr,
				char *buf) {
	return sprintf(buf, "%d\n", hacky_v1_offset[1]);
}

static ssize_t green_v1_offset_store(struct device *dev, struct device_attribute *attr,
				const char *buf, size_t size) {
	int value;
	if (sscanf(buf, "%d", &value) == 1) {
...
static ssize_t blue_v1_offset_show(struct device *dev, struct device_attribute *attr,
				char *buf) {
	return sprintf(buf, "%d\n", hacky_v1_offset[2]);
}

static ssize_t blue_v1_offset_store(struct device *dev, struct device_attribute *attr,
				const char *buf, size_t size) {
	int value;
	if (sscanf(buf, "%d", &value) == 1) {
...
static ssize_t voodoo_color_version(struct device *dev, struct device_attribute *attr,
				char *buf) {
...
static int __devinit tl2796_probe(struct spi_device *spi) {
	...
	if (sysfs_create_group(&voodoo_color_device.this_device->kobj,
			&voodoo_color_group) < 0) {
